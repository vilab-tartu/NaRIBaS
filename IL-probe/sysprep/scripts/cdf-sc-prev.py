...
for i in range(len(dist)):
  ...
  f = open(str(dist[i])+'/charge_radial_2D.dat', 'w+')
  ...
  #axis = int(min(cube_an.NY,cube_ca.NY)/10)*10
  axis=40 #2nm
  ...
  for j in range(axis):
    ...
    for k in range(cube_an.NY):
      for l in range(cube_an.NX):
        ...
        d_an.append(cube_an.data[l][k][j+cube_an.NZ/2-axis/2]*n_an/sum_an)
      rd_an = zip(r_an,d_an)
    ...
    for k in range(cube_ca.NY):
      for l in range(cube_ca.NX):
        ...
        d_ca.append(cube_ca.data[l][k][j+cube_ca.NZ/2-axis/2]*n_ca/sum_ca)
      rd_ca = zip(r_ca,d_ca)
  ...
  filename = 'charge_radial_2D.dat'  
  name = 'charge_radial_2D'            #get file name without extension
  ...
  for line in fileinput.FileInput('simple_contour_plot.m',inplace=1):
  ...
  matlab = 'matlab -nosplash -nodesktop -r simple_contour_plot'
