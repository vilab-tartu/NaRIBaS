...
for i in range(len(dist)):
  ...
  for j in range(len(repn)):
    ...
    mean.append(numpy.mean(force[0:]))                         #make an array of mean force value
    force[:] = []                                              #clear the list of force values

  avgf.append(numpy.mean(mean))
  stdf.append(numpy.std(mean))
  mean[:] = []
