...
temperature_annealing_name=$(echo ${current_temperature[0]} | awk '{print $2}')
...
fullpath_charge=$electrode_name/$cation_name$anion_name/$additive_name$mol_fraction/$probe_name/$temperature_name/$version_name/configuration/$pull_distance_start_name/$surfacecharge_name/ch
...
mkdir -p $dir_experiments/$fullpath_charge
mkdir -p $dir_temp/$fullpath_temp

cd $dir_experiments/$fullpath_charge
pwd
...
sed 's/SED_temperature_name_SED/'$temperature_name'/g' $dir_systempreparation/2_NVT_probe_charge_varTemp.mdp > 2_NVT_probe_charge.mdp
sed -i 's/SED_temperature_anneal_name_SED/'$temperature_annealing_name'/g' 2_NVT_probe_charge.mdp

#Move everything to the rundirectory
mv 2_NVT_probe_charge.mdp electrode_local.itp topol_local.top $dir_experiments/$fullpath_charge/

cp $dir_experiments/$fullpath_start/NVT_lowtimestep.gro $dir_experiments/$fullpath_charge/
cp $dir_experiments/$fullpath_start/index.ndx $dir_experiments/$fullpath_charge/

####################################

#grompp and initiate production run
gmx grompp -f 2_NVT_probe_charge.mdp -c NVT_lowtimestep.gro -p topol_local.top -n index.ndx -o NVT_charge -maxwarn 1
gmx mdrun -deffnm NVT_charge
rm *#
