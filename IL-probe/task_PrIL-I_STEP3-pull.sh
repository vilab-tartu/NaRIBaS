...
mkdir -p $dir_experiments/$fullpath_pull
...
sed 's/SED_temperature_name_SED/'$temperature_name'/g' $dir_systempreparation/3_NVT_probe_pull_varTemp.mdp > 3_NVT_probe_pull.mdp
sed -i 's/SED_temperature_anneal_name_SED/'$temperature_annealing_name'/g' 3_NVT_probe_pull.mdp

#Move everything to the rundirectory
mv 3_NVT_probe_pull.mdp electrode_local.itp topol_local.top $dir_experiments/$fullpath_pull/

cp $dir_experiments/$fullpath_charge/NVT_charge.gro $dir_experiments/$fullpath_pull/

#Prepare the index file
gmx make_ndx -f NVT_charge.gro -o index.ndx << EOF
keep 0
r $electrode_left
name 1 Cathode
r $electrode_right
...
gmx grompp -f 3_NVT_probe_pull.mdp -c NVT_charge.gro -p topol_local.top -n index.ndx -o NVT_pull -maxwarn 1
gmx mdrun -deffnm NVT_pull
rm *#
