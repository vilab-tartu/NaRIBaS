...
fullpath=$electrode_name/$cation_name$anion_name/$additive_name$mol_fraction/$probe_name/$temperature_name/$version_name/$pull_distance_start_name/$surfacecharge_name/$distance_name/$replica_name
fullpath_pull=$electrode_name/$cation_name$anion_name/$additive_name$mol_fraction/$probe_name/$temperature_name/$version_name/configuration/$pull_distance_start_name/$surfacecharge_name/$distance_name
...
sed 's/SED_temperature_name_SED/'$temperature_name'/g' $dir_systempreparation/4_NVT_probe_anneal_varTemp.mdp > 4_NVT_probe_anneal.mdp
sed -i 's/SED_temperature_replica_name_SED/'$temperature_annealing_name'/g' 4_NVT_probe_anneal.mdp

#Move everything to the rundirectory
mv 4_NVT_probe_anneal.mdp electrode_local.itp topol_local.top $dir_experiments/$fullpath/

cp $dir_experiments/$fullpath_pull/NVT_pull.gro $dir_experiments/$fullpath/
...
#cp NVT.gro NVT_pull.gro
#grompp and initiate production run
gmx grompp -f 4_NVT_probe_anneal.mdp -c NVT_pull.gro -p topol_local.top -n index.ndx -o NVT_anneal -maxwarn 1
gmx mdrun -deffnm NVT_anneal
rm *#
