...
sed 's/SED_temperature_name_SED/'$temperature_name'/g' 2_NPT_varTemp.mdp > 2_NPT.mdp
...
mv 2_NPT.mdp topol_local.top $dir_experiments/$fullpath_start/
...
gmx grompp -f 2_NPT.mdp -c NVT_lowtimestep.gro -p topol_local.top -n index.ndx -o NPT_eq -maxwarn 1
rm mdout.mdp
...
cp NPT_eq.tpr $dir_temp/NPT_eq$runningnumber.tpr
if [ $totalnumberofsetups -eq 1 ]; then
rm $dir_temp/copyback.txt
fi
pathtocopyback=$(pwd)
echo "mv NPT_eq$runningnumber.tpr $pathtocopyback/NPT_eq.tpr" >> $dir_temp/copyback.txt
