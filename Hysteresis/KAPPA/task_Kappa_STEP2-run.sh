...
####################################
# Calculations
xbox=$(awk "BEGIN {print $xbox_nm*10.0}")       # in Å
ybox=$(awk "BEGIN {print $ybox_nm*10.0}")       # in Å
zbox=$(awk "BEGIN {print $zbox_nm*10.0}")       # in Å
xbox12=$(awk "BEGIN {print $xbox*0.5}")
ybox12=$(awk "BEGIN {print $ybox*0.5}")
zbox12=$(awk "BEGIN {print $zbox*0.1}")
r_wall=$(awk "BEGIN {print $r_wall_nm*10.0}")   # in Å
z1_ion=$(awk "BEGIN {print $zbox12+$r_wall}")   # in Å
z2_ion=$(awk "BEGIN {print $zbox12+5*2+$r_wall}") # in Å

A_box=$(awk "BEGIN {print $xbox*$ybox}")        # box area in Å²
A_ion=$(awk "BEGIN {print 100/$dens_ion_nm2}")  # ion area in Å²
maxnumberofions_name=$(awk "BEGIN {print int($A_box/$A_ion)}")
varnumberofions_name=$(awk "BEGIN {print $maxnumberofions_name + 1.0*$numberofions_name}")
electrode_atom_charge=$(awk "BEGIN {printf \"%.20f\n\", -1.0*$ion_charge_name*$varnumberofions_name/$electrodeatoms}")
...
# Move to system-prep directory to build the “full” MDP
cd $dir_systempreparation

# Create 2_NVT_run.mdp (includes energygrps = Ion)
...
# Copy in the .gro and .top files so grompp can find them
cp $dir_systempreparation/NVT_lowtimestep.gro .
cp $dir_systempreparation/topol_local.top .
...
if [ -f NVT.gro ]; then
    ...
    #### GPU‐accelerated run (no energy groups) ####
    # Strip out the energygrps line
    sed '/^[[:space:]]*energy[_]*grps[[:space:]]*=/d' 2_NVT_run.mdp > 2_NVT_gpu.mdp

    rm -f NVT.tpr NVT.xtc NVT.edr mdout.mdp
	gmx grompp \
      -f 2_NVT_gpu.mdp \
      -c NVT_lowtimestep.gro \
      -p topol_local.top \
      -n index.ndx \
      -o NVT \
      -maxwarn 1
    ...
    gmx mdrun \
      -ntmpi 1 -ntomp 8 -pin off \
      -deffnm NVT

    #### CPU‐only rerun (with Ion energy group) ####
    # Regenerate the TPR including energygrps = Ion
    gmx grompp \
      -f 2_NVT_run.mdp \
      -c NVT_lowtimestep.gro \
      -p topol_local.top \
...
gmx potential \
  ...
  -n index.ndx \
  -sl 2000 \
  -o potential_$varnumberofions_name.xvg <<EOF
0
q
EOF

# Extract number density profile
gmx density \
  -f NVT.xtc \
  -n index.ndx \
  -s NVT.tpr \
  ...
  -dens number \
  -b 10 \
  -o density_$varnumberofions_name.xvg <<EOF
3
EOF
